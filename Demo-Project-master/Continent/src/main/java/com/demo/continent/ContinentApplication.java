...
public class ContinentApplication {
	...
	CommandLineRunner runner(CountryService countryService){
	    return args -> {
			// Load JSON and read it to create tables in Postgres
			...
			try {
				...
				System.out.println("Continents Saved Successfully!");
			} catch (IOException e){
				System.out.println("Error to save continents: " + e.getMessage());
			}
	    };
	}
